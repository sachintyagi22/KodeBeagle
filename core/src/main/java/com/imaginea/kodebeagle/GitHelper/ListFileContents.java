...
public class ListFileContents {
	...
	public static Map<String, Integer> fileRankings = new HashMap<String, Integer>();
	public static final String fileName = "core/src/main/scala/com/kodebeagle/crawler/ZipBasicParser.scala";
	...
	public static void main(String args[]) throws IOException, GitAPIException {
		...
		SortRankings.sortRankingsMap(fileRankings);
	 ...
	 * @throws IOException
	 */
	public static void walkAllCommits(Repository repo) throws GitAPIException, IOException {
		...
		for (Ref branch : branches) {
			...
			for (RevCommit commit : commits) {
				...
				if (foundInThisBranch) {
					List<PathModel.PathChangeModel> filesList = getFilesInCommit(repo, commit, true);
					for (int i = 0; i < filesList.size(); i++) {
						if (filesList.get(i).name.equals(fileName)) {
							rankFilesCommitLevel(filesList, fileRankings);
						} else {
	 ...
	 * @param fileRankings
	 */
	public static void rankFilesCommitLevel(List<PathModel.PathChangeModel> filesList, Map<String, Integer> fileRankings) {
		for (int i = 0; i < filesList.size(); i++) {
		...
		try {
			...
			if (commit.getParentCount() == 0) {
				...
				while (tw.next()) {
					...
					list.add(new PathModel.PathChangeModel(tw.getPathString(), tw.getPathString(), filestoreItem, size,
